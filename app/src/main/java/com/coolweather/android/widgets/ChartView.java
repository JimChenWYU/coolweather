...
public class ChartView extends View {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        int widthScale = (int) (width / (2 * minTemp.size()));
        ...
        for (int i = 0; i < minTemp.size(); i++) {
            ...
            canvas.drawCircle((2 * i + 1) * widthScale, (maxValue - minTemp.get(i)) * heightScale, 5, paint);
            String valueStr = String.valueOf(minTemp.get(i)) + "℃";
            paint.getTextBounds(valueStr, 0, valueStr.length(), rect);
            float textHeight = rect.height();
            float textWidth = rect.width();
            canvas.drawText(valueStr, (2 * i + 1) * widthScale - textWidth / 2, (maxValue - minTemp.get(i)) * heightScale + textHeight + 20, paint);
        }
